...
class AugFPNv5(nn.Module):
    ...
    def __init__(self,
        ...
        self.attention_conv = nn.conv2d(out_channels, 2, kernel_size=1)
        ...
        # self.stages.extend(
        #     [nn.Sequential(F.adaptive_avg_pool2d(output_size=(size, size)),
        #                    nn.Conv2d(in_channels[-1], out_channels, kernel_size=1, bias=False)) for size in
        #      self.psp_pool_output_size])
        self.stages.extend(
            [nn.Conv2d(in_channels[-1], out_channels, kernel_size=1, bias=False) for k in range(len(self.psp_pool_output_size))]
        )
        self.bottleneck = nn.Conv2d(out_channels * (len(self.psp_pool_output_size)), out_channels, kernel_size=1)
        self.relu = nn.ReLU()

        self.psp_bottle_attention = nn.Sequential(
            nn.Conv2d(out_channels * (len(self.psp_pool_output_size)), out_channels, 1), nn.ReLU(),
            nn.Conv2d(out_channels, len(self.psp_pool_output_size), 3, padding=1))
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1):
            ...
            "对权重map下采样可能有问题，需要实验看看结果"

            #############loss attention##############
            att_mask[i + 1] = self.attention_conv(laterals[i + 1])
            att_mask_softmax = nn.Softmax(att_mask[i + 1], dim=1)
            att = att_mask_softmax[:, 1:, :, :]
            laterals[i + 1] = att * laterals[i + 1]
...
#######################添加PSP#######################################
        PSP_Features = [F.upsample(self.stages[j](F.adaptive_avg_pool2d(inputs[-1], output_size=(self.psp_pool_output_size[j], self.psp_pool_output_size[j]))),
                                   size=(h, w), mode='bilinear', align_corners=True) for j in range(len(self.psp_pool_output_size))]

        # priors = [F.upsample(input=stage(inputs[-1]), size=(h, w), mode='bilinear') for stage in self.stages] + [
        #     inputs[-1]]
        # print(inputs[-1].size())
        bottle = self.bottleneck(torch.cat(PSP_Features, dim=1))
        bottle = self.relu(bottle)
        adap_pool_fusion = adap_pool_fusion + bottle
        ...
        return tuple(outs), tuple(att_mask)
