...
class AugFPNv3(nn.Module):
    ...
    def __init__(self,
        ...
        #### 最底层加了个残差增强模块#####
        self.high_lateral_bottom_conv = nn.ModuleList()
        self.high_lateral_bottom_conv.extend(
            [nn.Conv2d(in_channels[0], out_channels, 1) for k in range(len(self.adaptive_pool_output_ratio))])
    ...
    def forward(self, inputs):
        ...
        #######################在底层加一个RFA##################
        h, w = inputs[0].size(2), inputs[0].size(3)   #最底层网络
        #  Ratio Invariant Adaptive Pooling
        AdapPool_Features = [F.upsample(self.high_lateral_bottom_conv[j](F.adaptive_avg_pool2d(inputs[0], output_size=(
        ...
        laterals[0] += adap_pool_fusion  # M6+M5=P5
