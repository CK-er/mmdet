    ...
    def get_target(self, sampling_results):
        ...
        for j in range(map_size):
            ...
            for i in range(map_size):
                ...
                targets[:, 0, j, i] = x - pos_gt_bboxes[:, 0]     # l
                targets[:, 1, j, i] = pos_gt_bboxes[:, 2] - x     # r
                targets[:, 2, j, i] = y - pos_gt_bboxes[:, 1]     # t
                targets[:, 3, j, i] = pos_gt_bboxes[:, 3] - y     # b             targets = (l,r,t,b)     四维张量（nums_rois, 4, 7, 7）

                idx = ((x-pos_gt_bboxes[:, 0] >= dx) & (pos_gt_bboxes[:, 2] - x >= dx) & (y - pos_gt_bboxes[:, 1] >= dy) & (pos_gt_bboxes[:, 3] - y >= dy))     # m
                ...
                points[:, 3, j, i] = pos_bboxes[:, 3] - pos_bboxes[:, 1]          # points = (x,y,w,h)   四维张量（num_rois, 4, 7, 7）
