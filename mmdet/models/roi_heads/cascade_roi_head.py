...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_dummy(self, x, proposals):
        # bbox head
        outs = ()
        rois = bbox2roi([proposals])      #deal with proposals bbox to roi
        if self.with_bbox:
            for i in range(self.num_stages):
    ...
    def forward_train(self,
        ...
        """
        losses = dict()
        for i in range(self.num_stages):                       # num_stage = 3   cascade rcnn 1 stage + 3 stage, 三次循环
            self.current_stage = i                             # 3 stage rcnn for detect
            rcnn_train_cfg = self.train_cfg[i]                 # 不同stage, rcnn的参数不一样
            lw = self.stage_loss_weights[i]                    # stage_loss_weights=[1, 0.5, 0.25]

            # assign gts and sample proposals      分正负样本  assign()，     采样候选框  sample()
            sampling_results = []
            if self.with_bbox or self.with_mask:                                   # if include bbox or mask  -> true
                bbox_assigner = self.bbox_assigner[i]
                bbox_sampler = self.bbox_sampler[i]
                num_imgs = len(img_metas)                                          # 猜测是输入图像批次
                if gt_bboxes_ignore is None:
                    gt_bboxes_ignore = [None for _ in range(num_imgs)]             # 生成num_imgs个 none 值
            ...
            bbox_results = self._bbox_forward_train(i, x, sampling_results,              #  调用前面的_bbox_dorward_train前向函数
                                                    gt_bboxes, gt_labels,
                                                    rcnn_train_cfg)
