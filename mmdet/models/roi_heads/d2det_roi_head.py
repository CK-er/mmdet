...
class D2DetRoIHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
                            img_metas, gt_masks):
        bbox_results = super(D2DetRoIHead,                                     #  bbox_results = ('cls_score', 'bbox_pred', 'bbox_feats')
        ...
        # reg_feats = （num_feats, 256, 14, 14）
        if self.with_shared_head:
            reg_feats = self.shared_head(reg_feats)
        # Accelerate training
        max_sample_num_reg = self.train_cfg.get('max_num_reg', 192)
        sample_idx = torch.randperm(
            reg_feats.shape[0])[:min(reg_feats.shape[0], max_sample_num_reg)]       # torch.randperm 随机洗牌, 把0到reg_feats.shape[0]这些数随机打乱得到一个数字序列
        # print(sample_idx)
        reg_feats = reg_feats[sample_idx]
        # print(reg_feats.size(0))
        ...
        if self.MASK_ON == False:
            ...
            if reg_feats.shape[0] == 0:
                ...
                reg_pred, reg_masks_pred = self.D2Det_head(reg_feats)                                   # 对提取的特征图分别进行卷积得到四维和一维的特征图
                reg_points, reg_targets, reg_masks = self.D2Det_head.get_target(sampling_results)       # reg_points 局部特征的位置（x,y,w,h）, reg_targets 密集的框偏移量(l,r,t,b) , reg_masks 二元重叠预测 m
                ...
                y2 = reg_points[:, 1, :, :] + reg_pred[:, 3, :, :] * reg_points[:, 3, :, :]             # 利用最后卷积得到特征图作为权重对局部特征进行加权得到预测的偏移框

                pos_decoded_bbox_preds = torch.stack([x1, y1, x2, y2], dim=1)                  # 利用最后卷积得到特征图作为权重对局部特征进行加权得到预测的偏移框
                # pos_decoded_bbox_masks = pos_decoded_bbox_preds * reg_masks
                # proposal_second = pos_decoded_bbox_masks.permute(0, 2, 3, 1).reshape(-1, 4)
                # # pos_decoded_bbox_preds = pos_decoded_bbox_preds.permute(0, 2, 3, 1).reshape(-1, 4)
                # print(reg_feats.size())
                # print(pos_decoded_bbox_preds.size())
                # print(reg_masks.size())
                # print(pos_decoded_bbox_preds.permute(0, 2, 3, 1).reshape(-1, 4).size())
                # # print(pos_decoded_bbox_preds.permute(0, 2, 3, 1).reshape(-1, 4))
                # # print(proposal_second)
                # for i in range(proposal_second.shape[0]):
                #     if proposal_second[i, :] == [0, 0, 0, 0]:
                #         np.delete(proposal_second, i, axis=0)
                # return proposal_second
                # print(proposal_second)
                ...
                y2_1 = reg_points[:, 1, :, :] + reg_targets[:, 3, :, :]                                 # 密集真值框

                pos_decoded_target_preds = torch.stack([x1_1, y1_1, x2_1, y2_1], dim=1)                 # 密集真值框
                # print(pos_decoded_target_preds.size(0))
                # print(pos_decoded_target_preds.permute(0, 2, 3, 1).reshape(-1, 4).size(0))
                loss_reg = self.loss_roi_reg(
                    pos_decoded_bbox_preds.permute(0, 2, 3, 1).reshape(-1, 4),                # reshape(行数, 列数) 常用来更改数据的行列数目, -1 代表程序随机分配, 即表示任意整数, reshape(-1, 4), 表示 (任意行, 4列)
            ...
            loss_reg = self.loss_roi_reg(
                ...
                pos_decoded_target_preds.permute(0, 2, 3, 1).reshape(-1, 4),                     # reshape(行数, 列数) 常用来更改数据的行列数目, -1 代表程序随机分配, 即表示任意整数, reshape(-1, 4), 表示 (任意行, 4列)
                weight=reg_masks.reshape(-1))
            loss_mask = self.loss_roi_mask(
